...
class Solution:
    def isPalindrome(self, s: str) -> bool:
        ...
        while l < r: ## 左右の比較すべきペアがまだ残っているか？の判定
            ...
            while l < r and not self.alphaNum(s[l]): ## 英数字でない場合、比較の対象にならないかスキップする（lが英数字に当たるまで繰り返す）
                l += 1
                ## スキップした次の段階で英数字になっていれば、rのポインタの確認に進む
            while r > l and not self.alphaNum(s[r]): ## rが英数字に当たるまで繰り返す
                r -= 1
            if s[l].lower() != s[r].lower(): ## 英数字の場合は左右の英数字を比較。
                return False
            l, r = l + 1, r - 1 ## 一致したら: 次の比較のために内側にポインタを進める
        return True
